...
/**
 * Web search proxy Edge Function.
 * Queries DuckDuckGo HTML lite and returns parsed results as JSON.
...
/** Parse DuckDuckGo HTML lite results, ported from Rust tool.rs */
      ...
      if (he !== -1) {
        ...
        if (uddg !== -1) {
          ...
          href = decodeURIComponent(
            (end !== -1 ? encoded.slice(0, end) : encoded).replace(/\+/g, " "),
          );
        } else {
          href = raw;
        }
