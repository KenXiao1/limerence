      ...
      <section className="relative z-10 mx-auto max-w-6xl px-6 pb-24">
        ...
        >
          ...
          <div className="relative flex flex-col gap-8">
            {/* ── Row 1: Rust Crate Architecture ── */}
            <div>
              <div className="mb-4 flex items-center gap-2.5">
                <div className={`flex h-7 w-7 items-center justify-center rounded-lg ${
                  isDark ? "bg-amber-500/15" : "bg-amber-600/10"
                }`}>
              ...
              <div className="flex flex-col items-stretch gap-3 sm:flex-row sm:items-center">
                {[
                  ...
                  <div key={crate.name} className="flex flex-1 items-center gap-3 sm:gap-2">
                    <div className={`flex-1 rounded-xl border px-4 py-3 ${
                      isDark
                        ? "border-amber-500/20 bg-amber-500/[0.04]"
                        : "border-amber-600/15 bg-amber-600/[0.03]"
                    }`}>
                      <div className="mb-1.5 flex items-center gap-2">
                        <div className={`h-1.5 w-1.5 rounded-full ${isDark ? "bg-amber-500" : "bg-amber-600"}`}
                          style={{ boxShadow: isDark ? "0 0 6px #f59e0b" : "0 0 6px #d97706" }}
                        />
                        <span className={`font-mono text-[11px] font-medium ${isDark ? "text-amber-400" : "text-amber-700"}`}>
                          {crate.name}
                        </span>
                      </div>
                    ...
                    {/* Arrow between crates (not after last) */}
                    {i < 2 && (
                      <span className={`hidden text-sm sm:block ${isDark ? "text-zinc-600" : "text-zinc-400"}`}>
                        →
                      </span>
                    )}
                  </div>
            ...
            {/* ── Connector: TS Isomorphic Port ── */}
            <div className="flex items-center justify-center py-2">
              {/* Desktop: vertical dashed line with label */}
              <div className="hidden sm:flex sm:flex-col sm:items-center sm:gap-1">
                ...
                <span className={`rounded-full px-3 py-1 font-mono text-[10px] font-medium ${
              ...
              {/* Mobile: horizontal divider with label */}
              <div className="flex w-full items-center gap-3 sm:hidden">
                <div className={`h-px flex-1 ${isDark ? "bg-zinc-700" : "bg-zinc-300"}`} />
                <span className={`font-mono text-[10px] font-medium ${isDark ? "text-zinc-400" : "text-zinc-500"}`}>
                  TS 同构移植
                </span>
                <div className={`h-px flex-1 ${isDark ? "bg-zinc-700" : "bg-zinc-300"}`} />
              </div>
